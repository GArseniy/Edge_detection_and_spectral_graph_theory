...
def make_sparse_laplacian(lab, rad=1, activate_value=15):
    h, w = np.shape(lab)[:2]
    cnt_not_zeroes = 2 * ((2 * rad + 1) ** 2 - 1) * (w - 2 * rad) * (h - 2 * rad)
    ...
    it = 0
    for i in range(rad, h - rad):
        for j in range(rad, w - rad):
            for k in range(2 * rad + 1):
                for p in range(2 * rad + 1):
                    if k == rad and p == rad:
                        continue
                    ...
                    data[it] = 1 if ((i - rad + k) < rad or (i - rad + k) > h - 1 - rad
                                     or (j - rad + p) < rad or (j - rad + p) > w - 1 - rad) \
                        else 0.5
                    it += 1
...
def save_concat_image(filename, fiedler_vector, dt, act_val, alpha=0.15):
    ...
    im.save('Res/av-' + str(act_val) + '___now_time-' + str(time.time()).split('.')[0] + '___exec_time-' + str(dt)[
                                                                                                           :5] + "sec___" +
            filename.split('\\')[1])
...
if __name__ == '__main__':
    ...
    path_list = pathlib.Path(dir_name).glob('**/text_ege.png')
    for path in path_list:
        file_name = "Ref\\" + str(path).split('\\Ref\\')[1]

        color_activate_value = 15
